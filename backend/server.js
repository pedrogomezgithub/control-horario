...
// Configuración de CORS - PERMITIR TODAS LAS REQUESTS
const corsOptions = {
...
// Rate limiting DESHABILITADO para evitar bloqueos
// const limiter = rateLimit({
//   windowMs: parseInt(process.env.RATE_LIMIT_WINDOW_MS) || 15 * 60 * 1000,
//   max: parseInt(process.env.RATE_LIMIT_MAX_REQUESTS) || 100,
//   message: { success: false, message: 'Demasiadas solicitudes desde esta IP, intente de nuevo más tarde.' },
//   standardHeaders: true, legacyHeaders: false,
// });
// app.use(limiter); // COMENTADO PARA PERMITIR TODAS LAS REQUESTS
...
app.get('/', (req, res) => {
  res.json({
    success: true,
    message: '🎉 API de Control Horario FUNCIONANDO!',
    version: '1.0.0',
    status: 'READY',
    database: 'PostgreSQL configurada automáticamente',
    endpoints: {
...
// Endpoint de test simple que siempre funciona
app.get('/test', async (req, res) => {
  try {
    const empleados = await executeQuery('SELECT COUNT(*) as total FROM employees');
    const entradas = await executeQuery('SELECT COUNT(*) as total FROM time_entries');

    res.json({
      success: true,
      message: '✅ Sistema completamente funcional!',
      data: {
        empleados_en_bd: empleados[0].total,
        entradas_de_tiempo: entradas[0].total,
...
const startServer = async () => {
  try {
    ...
    // FORZAR SETUP COMPLETO DE BASE DE DATOS AL INICIAR
    console.log('🔧 Ejecutando setup automático de base de datos...');
    ...
    // También ejecutar el initializeDatabase por compatibilidad
